...
| **M1.5b** 项目 / 会话 / 任务（spec 08） | 语音侧只剩三个工具；"在哪个项目、开不开新会话、停哪个任务"由编码执行器自己判断；支持多项目并发；显式取消；会话有可读标题 | 🟡 代码与测试完成，两轮 review 的阻断项均已修并带测试；语音端到端与并发审批真机未验，**不勾**（见第四、五节） |
...
- 模型解析"停哪个"的期间用户改口了，不会停错（coordinator 路径和 `cancel` 工具路径都带这个检查）。
...
| runtime 单测 | 2054 个，2052 通过，2 平台跳过，0 失败（含 live eval；删文本前脑后从 2070 减少） |
...
- coordinator 决策 eval（真实 `qwen-flash`，已写成测试，有 key 时随套件一起跑）分两组：
  ...
  - **holdout** 10 条（prompt 冻结后才写、没用来调：口语填充词、ASR 把连字符念成空格、句内改口、前缀重名、状态提问、英文）。5 次运行 **8 / 超时 / 7 / 9 / 7**，门槛 ≥7。两个系统性失分：①"博客那个跑完了吗？"被判成 `steer`（会把问句当追加要求发给正在跑的 Codex；语音模型层本应直接从上下文回答不调工具）；②"pricing 那边的测试跑一下"在 `pricing-page` / `pricing-svc` 之间随机选一个。另有 2 次模型编造证据（"重新开个会话"被配上"博客"），**被宿主的证据校验拦下**——这正是为什么证据校验和确认必须是宿主的确定性逻辑而不是 prompt 约束。
...
| coordinator 模型的系统性失分 | 状态提问被判成 `steer`；前缀重名（`pricing`）随机选一个；偶发编造证据。都有宿主兜底（证据校验 + 确认），但会多一轮问答 | 下一版 prompt 要针对这两类修，并换一组新的 holdout |
...
3. **holdout 的两类系统性失分**（状态提问→steer、前缀重名）：接受"宿主兜底、多一轮问答"，还是要求 prompt 第二版把 holdout 提到 ≥9/10 再进真人验收？
