...
- 模型解析"停哪个"的期间用户改口了，不会停错：coordinator 路径看 intake revision，显式 `cancel` 工具路径还看桌面独立送达的 `local_speech_onset`，不受串行 provider 接收循环阻塞。
...
- 桌面端：审批弹窗带"哪个项目 / 哪个会话在问"；项目名单和正在跑的任务已经下发到渲染端，但还没画出来。
...
加上：桌面画出项目名单；有桌面的机器上跑 Electron smoke；针对状态提问 / 前缀重名出第二版 assess prompt 并换一组新 holdout。
