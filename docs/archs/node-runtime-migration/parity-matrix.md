...
| Memory recall and board projection | recall/memory-board tests | 19 Python-exported recall scenarios matching on both legs, including the encoded envelope at several budgets | Same bounded visible content; recall normalizes through the pinned Unicode pipeline, not the host's |
