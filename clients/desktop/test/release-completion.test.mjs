...
test('release candidate explicitly builds unsigned bytes and keeps trust-bound smoke checks', async () => {
  ...
  assert.match(workflow, /node clients\/desktop\/node_modules\/electron\/install\.js/u)
  assert.match(
    workflow,
    /npm run test:runtime\n\s+if: .*runner\.os != 'Windows'/u,
  )
  assert.match(workflow, /npm run test:runtime:win\n\s+if: runner\.os == 'Windows'/u)
  assert.match(workflow, /npm run test:desktop\n\s+if: inputs\.candidate_scope == 'full' \|\| matrix\.platform == 'win32'/u)
...
test('promotion is manual, candidate-bound, main-bound, and publishes npm only after release replacement', async () => {
  ...
  assert.match(
    workflow,
    /cp clients\/desktop\/scripts\/installed-candidate-smoke\.mjs candidate\/release-smoke-kit\/scripts\//u,
  )
...
test('promotion main refresh preserves candidate ancestry for split-run verification', {
  ...
  await mkdir(source)
  runGit(source, 'init')
  runGit(source, 'symbolic-ref', 'HEAD', 'refs/heads/main')
