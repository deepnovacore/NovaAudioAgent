...
test('optional large diameter scales both backing store and atlas with bounded size', () => {
  for (const [size, backing] of [[320, 640], [1000, 1024], [Infinity, 232]]) {
    const mounted = mount({size, devicePixelRatio: 2})
    assert.deepEqual([mounted.canvas.width, mounted.canvas.height], [backing, backing])
    assert.equal(mounted.offscreen[0].width, backing)
    mounted.visual.destroy()
  }
})

test('host can opt into freezing reduced motion and resume when preference changes', () => {
  const mounted = mount({freezeOnReducedMotion: true, reducedMotion: true})
  assert.equal(mounted.pending.length, 0)
  mounted.visual.setAccessibility({reducedMotion: false})
  assert.equal(mounted.pending.length, 1)
  mounted.visual.setAccessibility({reducedMotion: true})
