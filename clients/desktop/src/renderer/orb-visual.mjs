...
export function createOrbVisual(canvas, options = {}) {
  ...
  // the nebula canvas rotating unless the host opts into freezing it. Both
  // preferences remain live values so the
  ...
  const computeStaticMode = () => highContrastOn || (freezeOnReducedMotion && reducedMotionOn) || !schedule
  ...
  const diameter = Number.isFinite(size) ? Math.min(512, Math.max(1, size)) : ORB_SIZE
  const capRatio = raw => Math.min(MAX_PIXEL_RATIO, Math.max(1, raw)) * diameter / ORB_SIZE
  ...
  // API compatibility and freezes the nebula only when the host opts in.
