...
test('local start needs no runtime token and never sends credentials in hello', async t => {
  const {session, sockets} = await setup(t)
  await session.start()
  sockets.at(-1).onopen()
  assert.equal(sockets.at(-1).sent[0].type, 'hello')
  assert.equal(Object.hasOwn(sockets.at(-1).sent[0], 'token'), false)
})

test('failed authentication stops audio and cancellation fences pending runtime start', async t => {
  const {session, sockets, statuses} = await setup(t)
  globalThis.fetch.mock.mockImplementation(async () => ({ok: false, status: 401}))
  await session.start()
  assert.equal(session.active, false)
  assert.equal(statuses.at(-1).phase, 'error')
  let release
  globalThis.fetch.mock.mockImplementation(() => new Promise(resolve => { release = resolve }))
