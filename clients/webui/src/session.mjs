...
export function createSession({onStatus = () => {}, onCaption = () => {}, onState = () => {}, onLevel = () => {}, onSettings = () => {}} = {}) {
  ...
  let socket, connectionId, serverId, wanted = false, timer, attempts = 0, epoch = 0
  ...
  function stop() {
    wanted = false; epoch++; clearTimeout(timer)
    connectionId = null
  ...
  function connect() {
    ...
    current.onopen = () => {
      ...
      current.send(JSON.stringify({type:'hello', protocol_version:1, media:{transports:['host_pcm_v1']}}))
    }
  ...
  return {
    async start({credential: token, microphoneId} = {}) {
      stop()
      wanted = true; attempts = 0
      const started = ++epoch
      status('connecting', '正在启用麦克风…')
      try {
        await audio.start({microphoneId})
        if (started !== epoch || !wanted) return
        status('connecting', '正在启动 Nova…')
        for (const [path, body] of [['/api/session', {credential: token}], ['/api/runtime/start', {}]]) {
          const response = await fetch(path, {method: 'POST', credentials: 'same-origin', headers: {'Content-Type': 'application/json', 'X-Nova-WebUI': '1'}, body: JSON.stringify(body)})
          if (started !== epoch || !wanted) return
          if (!response.ok) {
            fail(response.status === 401 ? '请在设置中验证远程主机访问凭据。' : 'Nova 启动失败，请检查设置中的服务密钥和模型配置。')
            return
          }
        }
