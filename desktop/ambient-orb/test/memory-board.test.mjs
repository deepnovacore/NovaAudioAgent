...
test('board refresh preserves the current scroll positions', async () => {
  const source = await readFile(new URL('../src/renderer/memory-board.mjs', import.meta.url), 'utf8')

  assert.doesNotMatch(source, /createBoardAutoScroller|scrollAfterRefresh|autoScrollBottom/u)
})
