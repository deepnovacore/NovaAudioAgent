...
try {
  ...
  for (const factor of [1,1.25,1.5]) {
    ...
    await page.evaluate(()=>window.__frame({type:'codex.approval',pending_approval:true,pending_approval_busy:false,pending_approval_id:'approval',kind:'network',local_detail:{kind:'network',command:'npm install',cwd:'/workspace',scope:'网络：registry.npmjs.org'},operation_summary:'Codex 请求访问网络。',expires_in_seconds:60,allowed_decisions:['accept','acceptForSession','decline']}))
    await page.waitForFunction(()=>!document.querySelector('#codex-allow-session').hidden)
    await page.waitForTimeout(150)
    for(let i=0;i<3;i++) await page.evaluate(i=>window.__frame({type:'executor.progress',delegate_id:'d',executor:'codex',phase:'working',summary:['已开始处理任务','正在检查测试结果','已完成代码修改'][i],level:'milestone',ts:i+1}),i)
    ...
    await page.evaluate(()=>window.__frame({type:'codex.approval',pending_approval:false,pending_approval_busy:false,kind:null,local_detail:null,operation_summary:null,expires_in_seconds:null}))
    await page.locator('.progress-bubble').first().click()
    await page.locator('.progress-bubble').first().click()
    await page.waitForFunction(()=>document.querySelectorAll('.progress-bubble').length===0)
  }
  await page.evaluate(()=>window.__frame({type:'codex.approval',pending_approval:true,pending_approval_busy:false,pending_approval_id:'expiring',kind:'permissions',local_detail:{kind:'permissions',scope:'网络：请求访问'},operation_summary:'Codex 请求提升权限。',expires_in_seconds:.1,allowed_decisions:['accept','acceptForSession','decline']}))
  ...
  await page.evaluate(()=>window.__frame({type:'codex.approval',pending_approval:false,pending_approval_busy:false,kind:null,local_detail:null,operation_summary:null,expires_in_seconds:null}))
  await page.evaluate(()=>window.__frame({type:'executor.result',result:{delegate_id:'d',executor:'codex',outcome:'ok',summary:'修改完成并通过测试',started_at:0,ended_at:5,changed_files:2}}))
  await page.locator('#last-result').click()
  assert.equal(await page.evaluate(()=>window.__openedResult?.changedFiles),2)
  zoom=1
