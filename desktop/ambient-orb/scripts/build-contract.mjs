...
/** Emit plain browser ESM from the built runtime's single wire contract. */
export function wireFrameTypesSource(types) {
  return '// Generated by scripts/build.mjs from runtime desktop-wire; do not edit.\n'
    + `export const WIRE_FRAME_TYPES = Object.freeze(${JSON.stringify(types, null, 2)})\n`
    + `export const [${types.map(type => type.replaceAll('.', '_').toUpperCase()).join(', ')}] = WIRE_FRAME_TYPES\n`
}

export async function generateWireFrameTypes(root) {
  const {WIRE_FRAME_TYPES} = await import('@nova-audio-agent/runtime/desktop')
  await writeFile(resolve(root, 'src/renderer/wire-frame-types.mjs'), wireFrameTypesSource(WIRE_FRAME_TYPES))
}
