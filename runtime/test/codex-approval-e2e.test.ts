...
async function sendApprovalTool(
  ...
  await reserveUserDecision(service, input)
  await service.handleEvent({
...
test('Windows file approval crosses fake app-server, current Realtime user/tool authority, and terminal completion', async t => {
  ...
  assert.match(e2e.provider.injected.at(-1)?.content ?? '', /"code":"approval_accepted"/u)
})
...
test('a host tool arriving after its provider response terminal cannot accept the request', async t => {
  const e2e = await startApprovalE2e(t, 'file-approval-decline')
  ...
  assert.equal(e2e.controller.pending, true)
  assert.equal(e2e.service.codexApprovalDecision(approvalId, false), true)
  assert.equal(await within(e2e.factory.owner!.approvalDecision, 'late tool fallback decline'), 'decline')
