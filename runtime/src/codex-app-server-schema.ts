...
const LEGACY_COMMAND_APPROVAL_PARAM_FIELDS = deepFreeze({
  approvalId: 'string',
  command: 'string',
...
function validateApprovalSchemaSurface(bundle: Readonly<Record<string, unknown>>): void {
  ...
  const embeddedCommandKind = validateApprovalServerRequest(serverRequest)
  validateFileApprovalParams(
    requireObject(bundle['FileChangeRequestApprovalParams.json']),
  )
  const standaloneCommandKind = validateCommandApprovalParams(
    requireObject(bundle['CommandExecutionRequestApprovalParams.json']),
  )
  if (embeddedCommandKind !== standaloneCommandKind) throw new TypeError('approval kind shape')
...
function validateCommandApprovalParams(
  ...
  const hasKind = Object.hasOwn(properties, 'kind')
  validateExactObjectSchema(
    schema,
    hasKind ? COMMAND_APPROVAL_PARAM_FIELDS : LEGACY_COMMAND_APPROVAL_PARAM_FIELDS,
    APPROVAL_REQUIRED,
    root,
    COMMAND_APPROVAL_PARAM_TYPES,
  )
  if (hasKind) {
    const kind = requireObject(properties.kind)
    const kindChoices = kind.allOf
    if (!Array.isArray(kindChoices) || kindChoices.length !== 1 || kind.default !== 'command') {
      throw new TypeError('approval kind')
    }
    requireReference(
      requireObject(kindChoices[0]),
      '#/definitions/CommandExecutionApprovalKind',
    )
    const definitions = requireObject(root.definitions)
    const kindDefinition = requireObject(definitions.CommandExecutionApprovalKind)
    requireTypes(kindDefinition, root, ['string'])
    requireExactEnum(kindDefinition.enum, ['command', 'writeStdin'])
  }
