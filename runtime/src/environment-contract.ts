...
const rows: readonly Row[] = [
  ...
  ['NOVA_AUDIO_AGENT_SEARCH_PROVIDER', 'search', false, true, 'never', 'tavily', 'CLI or CI search provider override.', 'CLI 或 CI 搜索提供方覆盖。'],
  ['NOVA_AUDIO_AGENT_SEARCH_MCP_URL', 'search', false, true, 'never', null, 'Web search MCP endpoint override; unset uses the verified Bailian preset when MCP is selected.', '网页搜索 MCP 地址覆盖；选择 MCP 且未设置时使用已核对的百炼预设。'],
  ['NOVA_AUDIO_AGENT_SEARCH_MCP_TOOL', 'search', false, true, 'never', 'web_search', 'Web search MCP tool override (generic default web_search; Bailian preset bailian_web_search).', '网页搜索 MCP 工具覆盖（通用默认 web_search；百炼预设 bailian_web_search）。'],
  ['NOVA_AUDIO_AGENT_KNOWLEDGE_PATH', 'core', false, true, 'never', '~/.nova-audio-agent/knowledge.sqlite', 'Knowledge SQLite database path.', '知识库 SQLite 数据库路径。'],
  ['NOVA_AUDIO_AGENT_EMBEDDING_PROVIDER', 'core', false, true, 'never', 'dashscope', 'Knowledge embedding provider.', '知识库 embedding 提供方。'],
  ...
  ['NOVA_AUDIO_AGENT_MEMORY_TOKEN', 'core', true, true, 'when_selected', null, 'Host-issued identity-bound memory bearer token.', '宿主提供的身份绑定记忆令牌。'],
  ['NOVA_AUDIO_AGENT_MEMORY_PATH', 'core', false, true, 'never', '~/.nova-audio-agent/memory.sqlite', 'Personal memory database path.', '个人记忆数据库路径。'],
  ['NOVA_AUDIO_AGENT_MEMORY_USER_ID', 'core', false, true, 'never', 'local', 'Stable personal memory user identity.', '稳定的个人记忆用户身份。'],
  ...
  ['NOVA_AUDIO_AGENT_CODEX_MANAGED_ROOT', 'codex', false, true, 'never', '~/.nova-audio-agent/workspaces', 'Managed project root.', '托管项目根目录。'],
