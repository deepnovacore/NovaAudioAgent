...
const FRONTEND_INSTRUCTIONS_BEFORE_CODEX_APPROVAL = [
  ...
  '先根据用户目标与当前上下文判断关系，不得把特定产品名、游戏名或示例实体写成固定路由规则。',
  '路由优先级：待确认 proposal 的决定 > 继续当前 workspace 的已有 Session >',
  '切换其他或身份未知的 workspace > 创建独立 workspace > 在当前 workspace 启动新 Session。',
  '当前没有 active workspace，且用户提出可独立交付的完整产品或仓库时，使用 create_workspace；',
  ...
  '开放式询问后，用户用名词短语补充一个可独立交付物时，应结合上下文视为创建请求；',
  'workspace 名称必须从本轮用户表达动态提取，不得依赖固定产品名称，也不要改写成问句复述。',
...
const FRONTEND_INSTRUCTIONS_AFTER_CODEX_APPROVAL = [
  '面对新的 Codex 编程任务，调用 codex__project 携带 work_order 草稿；由宿主 intake 判断是否需要澄清。',
  'intake_opened / intake_in_progress 表示正在整理需求，尚未派单；不得说已开始执行。',
  '宿主事实携带问题时，只问给定的那一个问题，不调用另一项编码任务；仓库技术栈、入口、测试命令交给 Codex 探索。',
  '宿主说 ready / planning / readback / committing 时，不自行追问；纯确认用给定 proposal_id 调用 codex__confirm_project_action。',
  '用户修改需求时保留新约束，旧 proposal 不再有效。用户回答宿主问题后等待宿主规划，不重复创建任务。',
  '列表、切换、创建空工作区等管理动作保持原工具路径。一轮只做一个 action。',
  '用户要求先讨论时可以回应；不得把探索性提问当成执行许可。',
