...
export async function openTaskDirectory(path: string, launch: (file: string, args: readonly string[]) => Promise<void> = (file, args) => new Promise((resolve, reject) => execFile(file, [...args], error => error !== null ? reject(new Error('task directory opener failed', {cause: error})) : resolve())), platform: string = process.platform, stillWanted: () => boolean = () => true): Promise<void> {
